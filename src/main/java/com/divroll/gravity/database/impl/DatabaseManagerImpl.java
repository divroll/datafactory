 ...
 * the License.
 */
package com.divroll.gravity.database.impl;

import com.divroll.gravity.properties.EmbeddedArrayIterable;
import com.divroll.gravity.bindings.EmbeddedEntityBinding;
...
public final class DatabaseManagerImpl implements DatabaseManager {
  ...
  public PersistentEntityStore getPersistentEntityStore(String dir,
    ...
    if (entityStore == null) {
      ...
      entityStore =
          PersistentEntityStores.newInstance(config, environment, "persistentEntityStore");
  ...
  public <T extends Comparable, B extends ComparableBinding> void registerCustomPropertyType(
    ...
    getPersistentEntityStore(dir, false).executeInTransaction(txn -> {
      ((PersistentEntityStore) txn.getStore().getCurrentTransaction())
          .registerCustomPropertyType(txn, clazz, binding);
    });
  }
